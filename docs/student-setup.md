...
2. **드라이버 준비:** 모든 GPU에서 `nvidia-smi`가 동작하고 버전이 580.65.06 이상이면 기존 드라이버를 유지합니다. 이는 준비 기준이며 호환성 인증은 아닙니다. 다른 계열은 추가 안내 후 실제 실행 검사로 판단합니다.
3. **설치가 필요한 경우:** Ubuntu의 `ubuntu-drivers devices`가 해당 GPU에 지원한다고 보고한 후보 중 **580 계열**을 선택합니다. 여러 GPU는 후보 교집합을 사용합니다. 가능한 경우 `580-open`, 아니면 `580`을 선택합니다. `ubuntu-drivers install nvidia:580-open` 또는 `nvidia:580`으로 설치합니다. 커널 모듈·서명 처리는 Ubuntu 도구가 담당합니다.
4. **재부팅:** 설치 후 자동 재부팅하지 않습니다. 작업을 저장하고 직접 재부팅한 뒤 같은 명령을 실행합니다. Secure Boot가 켜져 있으면 MOK 등록 화면을 완료해야 할 수 있습니다. 재부팅 후에도 실패하면 반복 설치하지 않고 원인 확인을 안내합니다.
...
| 드라이버가 없거나 너무 오래됨 | 지원 후보를 조사하고 설치 확인 요청 |
...
- 호스트 전체 자동 검사 285개 통과, Xacro 미설치로 1개 생략. 낮은 사양 허용, 드라이버 후보 교집합, 최신 추천 계열 배제, 재부팅 대기, 원격 Docker 차단, 기존 환경 유지, daemon 조회 실패 및 새 PC의 설치 순서를 모의 검사했습니다.
