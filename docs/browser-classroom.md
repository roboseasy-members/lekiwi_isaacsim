...
편집기용 비밀번호를 만드는 단계는 없습니다. 아래의 SSH 인증 후 브라우저에서 편집기를 바로 엽니다.
...
노트북의 **새 로컬 터미널**에 서버가 출력한 `ssh -N ...` 한 줄을 그대로 복사해 실행합니다. 출력 명령에는 실제 서버 Ubuntu 계정과 주소가 포함됩니다.
아래는 직접 작성할 때 참고하는 명령 형식입니다. `사용자명`은 **데스크탑 Ubuntu 로그인 계정**으로 바꾸고,
...
비밀번호 안내에 `사용자명@...`이 그대로 보이면 `Ctrl+C`로 취소하고 실제 서버 계정으로 다시 실행합니다.
노트북 Chrome에서 **http://127.0.0.1:8080**을 열면 별도의 편집기 로그인 없이 VS Code 기반 편집기가 표시됩니다.
8080은 데스크탑의 외부 네트워크에 공개하지 않으며, 노트북과 데스크탑 사이의 통신은 SSH로 암호화됩니다.
코드·터미널과 교재 미리보기 모두 같은 주소에서 작동하고 별도 인증서 설치는 필요하지 않습니다.
이 방식은 [code-server의 공식 SSH 접속 안내](https://coder.com/docs/code-server/guide#port-forwarding-via-ssh)를 따릅니다.

이전 버전의 비밀번호 입력이나 로그인 화면이 남아 있다면 실행기를 종료하고,
코드를 갱신한 서버에서 `./lekiwi remote setup-editor`로 편집기 이미지를 다시 빌드한 뒤 실행기를 시작합니다.
...
접속 문구를 직접 정하거나 입력하지 않습니다. 노트북 실행기가 서버 Ubuntu 계정으로 SSH 인증해 임시 키를 자동으로 받습니다.
노트북에서
...
학습 1회·모델 저장과 실제 시뮬레이터의 추론 시작·정지·재시작을 확인했습니다.
[ACT 실행 리허설 기록](act-rehearsal-20260913.md)의 조건과 결과를 참고하세요. 과제 성공률과 15쌍 동시 사용은 별도 검증입니다.
