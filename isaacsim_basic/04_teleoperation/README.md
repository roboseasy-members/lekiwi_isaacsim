...
촬영 환경에서는 Space가 Isaac 기본 재생 단축키에도 반응하여 **Pause 상태로 바뀌는 것**을 확인했습니다.
다음 조작 전 이동키를 모두 놓고 왼쪽 Play/Pause 아이콘을 확인합니다. 필요하면 Play를 누른 뒤, teleop에서는 R로 재활성화합니다.
패널의 `PhysX active` 문구만으로 현재 재생 상태를 판단하지 마세요.
...
손목 카메라는 3편의 기본 자세에서 가림이 확인됐습니다. 보정 전에는 손목 영상만으로 집기 위치를 판단하지 않습니다.
...
이번 과정은 조작과 화면 기록까지이며, 영상 동기화·LeRobot 데이터셋 녹화·학습·정책 실행은 포함하지 않습니다.
