...
현재 카메라 장착은 실측 TF가 없는 **임시값**이며 `calibrated=false`입니다.
본격적인 집기 시연을 쌓기 전에 [3편의 렌즈 중심·촬영 방향 확인](../03_robot_cameras/README.md)을 따라 장착을 보정해야 합니다.
이번 짧은 주행 실습에서는 가림을 관찰하고, 임시 장착값과 보정 여부가 manifest에 남는 것을 확인합니다.
...
Space는 베이스 정지와 Isaac Sim Pause에 함께 반응할 수 있으므로, 이 실습의 정상 종료에는 기록 패널의 Stop recording을 사용합니다.
실제 리더 조작의 정지·비활성화 절차는 4편을 따릅니다.

![기록 종료 후 저장 전 상태](images/03-unsaved.png)

설정한 시간은 **시뮬레이션 시간** 기준이며, 30 Hz에서 `설정 초 × 30`프레임이 되면 `UNSAVED`로 전환합니다. 자동 저장하지 않으므로 성공 여부를 확인한 뒤 Save episode를 누릅니다. `0`이면 자동 종료하지 않으며 **Stop recording**으로 끝냅니다. 양수로 설정해도 언제든 먼저 종료할 수 있습니다.
...
검사가 끝난 뒤에만 완료 manifest를 기록하고 `.partial`을 제거합니다.
...
동일한 시작 배치가 필요하면 시뮬레이터를 종료하고 저장한 코스의 layout.json으로 새 실행을 시작합니다.
...
두 카메라는 물리 진행을 잠시 멈추고 `delta_time=0`으로 함께 렌더링합니다.
캡처 중 물리 스텝과 시각이 바뀌지 않았는지, 두 카메라의 render reference가 같은지 확인합니다.
RGB에 기록한 simulation_time은 이 동기 캡처 시점의 물리 시각입니다. 렌더러의 별도 시계를 물리 시각으로 간주하지 않습니다.
...
- 임시 카메라 장착과 단순 주행 기록을 집기 학습용 완성 시연으로 혼동하지 않습니다.
