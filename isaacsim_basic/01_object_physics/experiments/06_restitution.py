"""1장 · 받침대 위에 두 공을 떨어뜨려 반발계수를 비교하는 코드.

목적: 재질의 반발계수가 접촉 뒤 튀어 오르는 정도에 미치는 영향을 관찰합니다.
실행: 저장소 최상위 폴더 터미널에서 ./lekiwi basic --experiment 6

읽는 순서
1. build_scene(): Low/High 받침대와 같은 반지름·질량·시작 높이의 공을 만듭니다.
2. material(): 반발계수를 Low=0.0, High=0.8로 정합니다.
3. bind(): 같은 재질을 공과 받침대 양쪽에 연결합니다.
4. main(): 새 장면을 열고 Play 입력을 기다립니다.

기본 결과: High 쪽 공이 첫 충돌 뒤 더 높이 튀는지 관찰합니다.
바꿔 볼 값: build_scene() 마지막의 High 반발계수 0.3 줄 주석을 해제합니다.
반발계수 0.8은 처음 높이의 80%까지 튄다는 뜻이 아닙니다. 첫 반동을 비교하세요.
...
def build_scene(stage):
    ...
    for name, x, restitution, color in (("Low", -.5, 0.0, (.95, .5, .1)),
        ...
        sphere = UsdGeom.Sphere.Define(stage, f"/World/Ball{name}")  # 지정 경로에 구를 만듭니다. 반동을 비교할 물체입니다.
        sphere.CreateRadiusAttr(.05)  # 구의 반지름입니다. 이 장면에서는 m 단위입니다.
        sphere.AddTranslateOp().Set(Gf.Vec3d(x, 0, 1))  # 부모 좌표계 기준 위치 X/Y/Z를 m로 지정합니다.
        sphere.CreateDisplayColorAttr([Gf.Vec3f(*color)])  # 표시 색상 RGB를 각각 0~1로 설정합니다. 물리 마찰에는 영향이 없습니다.
        UsdPhysics.RigidBodyAPI.Apply(sphere.GetPrim())  # 객체에 동적 강체 속성을 추가해 중력·외력으로 움직일 수 있게 합니다.
        UsdPhysics.CollisionAPI.Apply(sphere.GetPrim())  # 형상을 접촉 계산에 포함합니다. 접촉하는 상대 물체에도 Collider가 필요합니다.
        UsdPhysics.MassAPI.Apply(sphere.GetPrim()).CreateMassAttr(.1)  # 강체 질량을 kg로 지정합니다. 0은 중력 OFF가 아니라 자동 계산 의미입니다.
        bind(pad, mat)
        bind(sphere.GetPrim(), mat)

    # 다음 단계: 높은 반발계수를 .3으로 낮춥니다. 범위: 0~1
    # UsdPhysics.MaterialAPI(stage.GetPrimAtPath("/World/Materials/High")).CreateRestitutionAttr(.3)
