...
먼저 마우스 실습의 결과를 화면에서 확인하고, 마지막에 같은 조건의 코드 실행 결과와 비교합니다.
...
일반 설치에서는 본인이 쓸 수 있는 폴더를 사용합니다. 이름이 이미 있으면 새 이름으로 저장합니다.
...
무거운 물체가 더 빨리 떨어졌는지, 달 중력에서 두 물체가 어떻게 달라졌는지 비교합니다.
...
어느 큐브가 더 미끄러지는지 관찰합니다. Stop 후 Low를 `0.05`로 복원하고 `my_friction.usda`로 저장합니다.
...
첫 반동의 높이를 비교하고, Stop 후 High를 `0.8`로 복원하여 `my_bounce.usda`로 저장합니다.
...
본인이 저장한 파일을 열어 속성값과 Play 결과를 다시 확인합니다.
