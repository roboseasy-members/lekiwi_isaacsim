...
1~3장 README에서 **마우스로 객체 생성 → 속성 변경·Play 관찰 → 저장·정리 → 코드 구현** 순서로 진행합니다.
4장부터는 준비된 빨주노초 코스에서 **텔레옵 → 시연 취득 → 변환·ACT 학습·추론**으로 이어갑니다.
...
1~3장 코드 확인은 `./lekiwi basic --chapter 1`~`3`으로 실행합니다.
4장은 `./lekiwi teleop ...`, 5장은 같은 명령에 `--record`, 6장은 해당 학생 파일을 호스트 `python3`로 실행합니다.
학생 experiments 파일은 저장 후 재실행하면 반영됩니다. 공통 `isaac_sim` 코드를 수정할 때는 교재 안내에 따라 이미지를 다시 빌드합니다.
...
| 4 | 빨주노초 코스의 키보드 베이스·SO101 리더암 텔레옵과 집기 연습 |
| 5 | 같은 코스에서 두 RGB·상태·명령 시연 취득과 에피소드 저장 |
| 6 | 원본 선택·LeRobot 변환·검사·선택 업로드·ACT 학습·추론 |

0장은 준비 교재·출처·노션 ZIP, 1~6편은 MD·학생 Python·실제 화면·노션 ZIP을 포함합니다. 실습 결과는 각 장 README의 안내에 따라 화면과 저장 파일에서 확인합니다.
1~3장 API 예제는 일반 Isaac Sim 5.1 설치에서도 실행할 수 있습니다. 4~6장은 프로젝트 코스·리더·기록·학습 모듈을 사용합니다.
교재 전체 목차에 6시간 핵심 과정 편성 예시가 있습니다. 최초 설치·보정 시간은 장비에 따라 달라지며 실제 리허설에서 조정합니다.
학습용 시연은 5장에서 취득합니다. 이전 단일 관절 입력·JSON 예제는 보충 자료로 보관합니다.
...
[5장의 기록 설정 안내](isaacsim_basic/05_data_recording/README.md#7-기록-스크립트에서-기본값을-바꾸는-방법)에 따라 공통 기록 코드의 기본값을 수정합니다. 시간 0은 F6을 누를 때까지 수동 수집합니다.
...
`.partial` 폴더는 미완료 기록입니다. 자세한 항목과 오류 처리는 [5장](isaacsim_basic/05_data_recording/README.md)을 확인하세요.
...
[6장](isaacsim_basic/06_lekiwi_dataset/README.md#5-act-학습이-실행되는지-확인하기)의 `06_train_act.py`에서 실제 데이터셋 이름과 새 `run_name`을 저장합니다.
